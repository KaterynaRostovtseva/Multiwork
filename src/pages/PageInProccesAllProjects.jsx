...
const PageInProccesAllProjects = () => {
    ...
    const inProccesProjects = dataAllProjects
        .filter(i => i.project_status === 'In Procces')
        .filter(project => {
    ...
    const totalPages = Math.ceil(inProccesProjects.length / itemsPerPage);
    const currentData = inProccesProjects.slice(
        ...
        <Grid container sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, marginTop: '-108px' }}>
            ...
            <Grid sx={{ flex: 1, margin: { xs: '138px 16px 0 16px', md: '138px 80px 80px 80px' } }}>
                <Grid sx={{ display: 'flex', justifyContent: 'space-between', flexDirection: { xs: 'column', md: 'row' } }}>
                    <Typography sx={{ ...textStylesH2, marginBottom: '40px' }}> In Procces projects</Typography>
...
export default PageInProccesAllProjects;
